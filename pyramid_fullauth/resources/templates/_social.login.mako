...
% if 'social' in request.registry["fullauth"]:
    % if 'facebook' in request.registry["fullauth"]["social"]:
        <a href="${social.social_auth_uri('facebook', scope=request.registry["fullauth"]["social"]["facebook"]["scope"])}">${_('Connect with facebook', domain='pyramid_fullauth')}</a>
    % endif
    % if 'twitter' in request.registry["fullauth"]["social"]:
